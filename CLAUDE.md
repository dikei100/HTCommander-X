...
- **TlsCertificateManager**: Self-signed X.509 certificate generation and caching for TLS. Generates RSA 2048 cert with SAN entries for `localhost`, `127.0.0.1`, `::1`, and all local network IPs. Stored as PFX at `{configDir}/htcommander-tls.pfx`, 10-year validity. Static `GetOrCreateCertificate(configDir)` method with thread-safe caching — both WebServer and McpServer share the same instance.
