...
When `ServerBindAll` is enabled, the MCP server requires a `Authorization: Bearer <token>` header on all POST requests. The token is auto-generated (24 random bytes, base64) on first run and stored as `McpApiToken` in DataBroker device 0. `McpDebugToolsEnabled` is **not** in the `SettingsWhitelist` — it can only be toggled from the Settings UI, preventing remote privilege escalation via `set_setting`. Even when debug tools are enabled, a `DebugSettingsBlacklist` (`McpApiToken`, `McpDebugToolsEnabled`, `TlsEnabled`) prevents modification of critical security settings via `set_app_setting` or `dispatch_event`.

### CORS origin validation
Both MCP and Web servers validate CORS `Origin` headers against an allowlist (localhost, loopback IPs, RFC 1918 private LAN ranges) via `ValidateCorsOrigin()`. Unknown or public origins are rejected. `Vary: Origin` header included for cache correctness.
...
- **Protocol bounds**: All radio data structure constructors validate minimum message length. `Radio.RadioTransport_ReceivedData` rejects messages < 4 bytes and catches `ArgumentException`. Channel ID bounds-checked before array access. `Utils.GetShort()` and `Utils.GetInt()` perform explicit bounds checking and throw `ArgumentException` on out-of-range positions.
- **Buffer caps**: AGWPE DataLen ≤ 65536, SMTP data ≤ 10MB, IMAP APPEND ≤ 10MB, Rigctld/CAT command lines ≤ 1024 bytes (disconnect on oversized), TlsHttpServer ≤ 100 concurrent connections, AudioClipHandler SaveAudioClip ≤ 10MB, WebAudioBridge audio frames ≤ 19200 bytes with per-client rate limiting (200 frames/sec).
- **Injection prevention**: All Linux subprocess calls (espeak-ng, pactl, pacat, parecord) use `ProcessStartInfo.ArgumentList` instead of string concatenation — prevents argument injection regardless of input content. HTTP response headers stripped of CRLF. IMAP tags sanitized; `ParseSequenceSet` and `ParseUidSet` use `TryParse` (not `Parse`) to handle malformed input gracefully. espeak-ng voice names filtered to `[a-zA-Z0-9_\-+]`. `SelfUpdateDialog` validates release URL via `Uri.TryCreate()` with host (`github.com`) and path (`/dikei100/HTCommander-X/releases/`) validation. `LinuxVirtualAudioProvider.LoadModule()` validates module names against an allowlist (`module-null-sink`, `module-virtual-source`).
...
### Error handling
Protocol error responses (SMTP, IMAP, MCP JSON-RPC) use generic messages — never expose `ex.Message` to clients. IMAP sequence set parsing uses `TryParse` to prevent `FormatException` crashes from malformed client input.
...
### Web security
Both `index.html` and `mobile.html` include Content-Security-Policy meta tags. CORS uses validated origin allowlist (localhost/loopback/private LAN) with `Vary: Origin`. All innerHTML assignments use `escapeHtml()`.

### Thread safety
`RadioAudioManager.OnTransmitVoicePCM` acquires `connectionLock` before accessing transport to prevent null reference races during disconnect. `LinuxRadioBluetooth.CreateRfcommFd` validates `bdaddr` length before array access to prevent fd leaks on malformed input.
