...
- **Frequency overflow protection**: All frequency-setting paths (RigctldServer, CatSerialServer, McpTools `set_vfo_frequency`) validate `freqHz > 0 && freqHz <= int.MaxValue` before casting `long` to `int` for `RadioChannelInfo.rx_freq`/`tx_freq`. Prevents integer overflow that could cause undefined radio behavior with frequencies > 2.1 GHz. RigctldServer `\set_freq` and CatSerialServer `FA`/`FB` validate `<= int.MaxValue` at parse time (before caching) to prevent bogus cached frequency values from being reported back to clients.
...
### Web security
Both `index.html` and `mobile.html` include Content-Security-Policy meta tags. CORS uses validated origin allowlist (localhost/loopback/private LAN/IPv6) with `Vary: Origin`. All innerHTML assignments use `escapeHtml()`. WebServer `/api/config` endpoint includes `Cache-Control: no-store` to prevent caching of sensitive configuration and provides `mcpToken` so the mobile web UI can authenticate with the MCP server when `ServerBindAll` is enabled. Mobile web UI (`mobile.js`) sends `Authorization: Bearer <token>` header on all MCP API calls and validates the MCP port (1-65535) before constructing URLs. WebServer uses canonicalized `fullPath` (from `Path.GetFullPath`) for `File.ReadAllBytes` to close TOCTOU gap with symlinks.
